...
- **理由**:① 对标调研结论(《10-research-and-selection-decisions.md》§3.1 品类全景、§3.4 对标启示 7)——计算(VM 形态产品、容器化实现先行,虚机由二期 KubeVirt 承接,见《09-roadmap.md》决策 R-03)+对象/块存储+VPC/负载均衡是最小可售骨架,托管数据库第二步;② IaaS 是其他一切产品(含自研中间件、大数据)的资源底座,先有底座才能自托管上层产品;③ 大数据/AI 依赖大规模资源池与生态运营,早期投入产出比最低。
