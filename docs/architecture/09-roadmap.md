...
> **本章权威性声明**:全书各章的**产品上架批次、阶段时间窗、部署拓扑演进与可用性门禁值**以本章为**唯一事实源**——尤其是一期产品集(决策 R-01)、部署形态(§2.3,与《00-overview.md》§4.2 P1/P2/P3 一一映射)与一期 GA 验收(§3.5)。其他章节的批次/排期/口径描述若与本章不一致,以本章为准并回写该章。
...
gantt
    ...
    二期 GA                        : milestone, m3, 2028-02-29, 0d
...
> 日期为示例锚点。实际排期以立项日 T0 平移,关键约束是"一期 9 个月、二期再 9 个月、三期 12 个月+"的节奏比例,而不是绝对日期。
...
> **决策 R-01:首批可售产品 = SCVPC(网络) + SCECS(云服务器 VM) + SCBS(块存储) + SCOSS(对象存储) + SCRDS(托管 MySQL) + SCMON(监控) + SCEIP(弹性公网 IP),共 7 个。SCECI(弹性容器实例)后置二期。**
>
> **理由**
...
> **决策 R-02:一期网络能力以 SCVPC(VPC+子网+安全组+弹性公网 IP)落地,基于 K8s 网络模型 + Calico/Cilium 承载。**
>
...
| 活跃用户 | 1 万 | 日活 |
| 在管资源实例数 | 5 万 | 计费资源口径 |
| 微服务数 | 17(一期) | 二/三期扩至 50+(见《03-backend-services.md》§4.0 总表) |
| Pod 数 | 约 200 | 管控面 |
...
| A4 | 可用性 | 控制面 ≥99.9%(月度),对象存储数据面 ≥99.95%,数据持久性目标 11 个 9 设计 | 压测+SLO 试算 |
...
| 云监控完整版 | 多租户指标与对客告警规则产品化(alert-engine+alert-center) | P1 | 一期仅自用,二期对外 |
...
二期基础设施扩容方向:MySQL 扩至 8×16 基线、Kafka 6 broker、ClickHouse 维持 6 节点(3 分片×2 副本)并按产品线拆库、VictoriaMetrics 以 remote_write 承接长周期指标(双写对比 ≥1 周后切流,见《10-research-and-selection-decisions.md》§4.4 兼容性坑清单)、APISIX 按业务域拆集群隔离爆炸半径。
...
timeline
    ...
    section 2027
        ...
        2027 Q3 : 弹性容器实例(SCECI)Beta : 计费四形态(资源包/抢占式)上线 : OpenAPI Explorer 发布
        2027 Q4 : 同城双 AZ(P2)演进完成 : 产品矩阵冻结评审
    section 2028
        2028 Q1 : 二期 GA : SLO 体系全量覆盖 : 第二地域选址开工
        2028 Q2 : 混沌工程平台上线 : 云市场 Alpha
        2028 Q3 : 跨地域数据复制上线 : 等保三级测评
        2028 Q4 : 双地域容灾演练
    section 2029
        2029 Q1 : 三期 GA : 生态门户正式运营
```
...
| 团队 | 一期(30~38 人) | 二期(55~70 人) | 三期(90~120 人) |
|---|---|---|---|
| 架构委员会 | 2(兼) | 3(兼) | 3(兼) |
...
> 人力口径说明:① 上表为研发侧编制,不含销售/市场/客服,商业化运营团队在三期前另行规划;② QA 随自动化成熟度逐步并入产品线小队;③ 每阶段人力增长率(~100%/~46%)与产品数增长率大致匹配,防止"人比产品先到"造成空转。
...
> **决策 R-05:发布节奏采用"双周发布列车 + 基础设施月度窗口 + 紧急热修随时"三级制。**
...
| 虚拟化 | 二期 **KubeVirt**(决策 R-03) | 复用 K8s 运维体系 | VM 规模 >5000 台或特性瓶颈时评估自研 IaaS 面 |
...
| 《08-devops-delivery.md》 | DevOps 与交付体系 | 发布列车(R-05)、GitOps 验收项与多地域 IaC 复制依据 |
