...
6. [MySQL 分库分表(ShardingSphere)](#6-mysql-分库分表shardingsphere)
...
| 领域 | 结论 | 核心理由 | 备选 | 改选条件 |
|---|---|---|---|---|
| API 网关 | **APISIX** | 全动态配置、插件热加载、限流/认证/灰度插件齐全、K8s 友好、语言中立 | Spring Cloud Gateway / Kong | 想压缩运维面且网关逻辑深度耦合 Spring 生态时选 SCG;需商业支持合同才考虑 Kong |
| 注册+配置 | **Nacos(一体化)** | 注册配置一套栈,SCA 原生集成、Go SDK 成熟,运维与权限减半 | Consul + Apollo | 需多数据中心联邦/服务网格选 Consul;需合规级变更审批 + IP 级灰度 + 数千配置项且已有 Apollo 运维经验才拆 Apollo |
| 消息队列 | **Kafka** | 候选池唯一 MQ;高吞吐、生态最全,计量/审计/日志类大流量场景天然适配 | (池内无) | — |
| 分库分表 | **ShardingSphere-JDBC** | Apache 顶级项目、无中心化代理、分布式事务生态完整 | ShardingSphere-Proxy | 出现多语言直连同一分片库需求时引入 Proxy(与 Kratos Go 域共存场景,见 6.10) |
...
flowchart TB
    ...
    subgraph "业务微服务 Java Spring Cloud / Go Kratos"
        BFF[BFF / 业务服务]
    end
...
- 理由:① 路由/插件全动态(etcd 热生效),发布不改配置不重启;② 内置 limit-count / limit-req / jwt-auth / traffic-split / prometheus 等插件覆盖本章全部诉求;③ 语言中立,同时承接 Java(Spring Cloud)与 Go(Kratos)上游;④ K8s Ingress 与原生 CRD/Helm 支持成熟。
- 备选:Spring Cloud Gateway —— 与 Spring+Nacos 同栈、运维面小;Kong —— 商业支持。
- 改选条件:团队明确放弃 Lua 运维能力且网关定制逻辑全部可用 Java 表达时改 SCG;采购合同要求商业 SLA 时评估 Kong Enterprise。
...
熔断策略:上游健康检查使用 APISIX 内置 `healthcheck`(被动检查:5xx/超时计数摘除,主动检查可选);**不引入独立熔断中间件**,服务级熔断由 Sentinel(Java 域)与 APISIX 插件(Go 域)分担,与选型表"避免双标准"一致。
...
- `service_name` 必须写成 `{GROUP}@@{serviceName}`;按本章 4.3 冻结的命名规范 Group 一律等于应用名,订阅串形如 `svc-order@@svc-order`,且 GROUP/namespace 必须与服务注册端(SCA `spring.cloud.nacos.discovery.group`、Kratos contrib 注册参数)完全一致;
...
- 理由:① 一套 HA 集群、一套鉴权与备份策略,运维成本减半;② Java(SCA)与 Go(nacos-sdk-go / Kratos contrib)双语言 SDK 均成熟,与 Java/Go 分工方案天然对齐;③ namespace/group 模型足以覆盖多环境与产品线分组;④ APISIX discovery 可直接对接。
- 备选:Consul(注册)+ Apollo(配置)。
- 改选条件:仅当**同时**满足——合规要求配置变更全量审批与审计、需要 IP 级灰度下发、配置项达数千且团队已有 Apollo 运维经验——才改为 Nacos 注册 + Apollo 配置。届时注意 Apollo Go SDK 为社区维护(open API + 长轮询降级),双栈意味着双倍运维面。
...
- **服务名**:管控域服务统一 `svc-{domain}`,短横线小写,Java 与 Go 注册名同构,如 `svc-iam`、`svc-order`;例外项以《03》§4.0 总表固化为名:接入层 BFF 采用 `{场景}-bff`(如 `console-bff`、`site-bff`)、`alert-engine`/`alert-center`、数据面资源控制器 `rc-*`;**产品控制面服务命名 `svc-{productCode}`**(productCode 见《01-product-catalog.md》D0,如 `scecs` 产品对应 `svc-scecs`)。注册元数据强制带 `lang=java|go`、`plane=mgmt|data` 标签,供网关与监控过滤;
...
| KMS 密钥服务 | `svc-kms` | Java | 安全 | 《07-security.md》§1.3(该章现名 `kms-service`,按本节规范改名) |
...
- 客户端监听规范:SCA `@RefreshScope` / Kratos config watch;约定**配置变更必须是幂等且可回滚的**,禁止依赖变更顺序的有状态配置;
...
**Producer 侧(统一 SDK 封装,Java/Go 双语言同参)**
...
## 6. MySQL 分库分表(ShardingSphere)

### 6.1 决策

**结论:ShardingSphere-JDBC(Jar 内嵌、无代理、无性能单点)承担分库分表与读写分离。**

- 理由:① Apache 活跃顶级项目,与 Spring Cloud / Hibernate / Druid 生态整合成熟;② JDBC 模式无额外组件,故障域与业务进程一致;③ 分布式序列、读写分离、(可选)分布式事务能力完整。
- 备选:ShardingSphere-Proxy。
- 改选条件:当 Kratos(Go)域服务需要**直接读写同一分片库**(而非经 Java 域服务 API)时,引入 Proxy 承担异构语言直连;此前 Go 域一律通过服务接口访问分片数据。

⚠ **兼容性**(选型表坑清单 #5):MySQL Connector/J 8.x 与 Hibernate 6.x 版本组合先在基础镜像中固化并回归;HikariCP `keepaliveTime` 开启防止长连接被中间设备掐断;不使用 Druid stat filter 与 ShardingSphere 叠加(曾误报连接泄漏)。

### 6.2 总体原则:先垂直拆库,再水平分片

```mermaid
flowchart TB
    subgraph "应用层 ShardingSphere-JDBC 内嵌"
        APP[业务服务]
    end
    subgraph "账号库 account_db"
        A1[user / auth / ram / ak<br/>按 account_id 分片]
    end
    subgraph "交易库 trade_db"
        T1[order / payment / bill / ledger / coupon<br/>按 account_id 分片]
    end
    subgraph "资源库 resource_db"
        R1[resource_instance 主档 + 各产品扩展<br/>按 account_id 分片]
    end
    subgraph "计量库 metering_db"
        M1[metering_detail 热数据90天<br/>按 account_id 分片 归档入CK]
    end
    subgraph "不分片库"
        C1[content_db 官网内容]
...
- **resource_id 单独分片的弊端**:用户控制台资源列表变成全分片扫描(128 片 scatter-gather),且订单→资源跨库事务增多。
...
### 6.5 分片配置示例(ShardingSphere YAML)

```yaml
# trade_db 分片规则(svc-order 内嵌 ShardingSphere-JDBC)
dataSources:
  trade_0: { url: jdbc:mysql://mysql-trade-0.mw-data:3306/trade_0, ... }
  # trade_0 ~ trade_7
...
- !SHARDING
  tables:
    t_order:
      actualDataNodes: trade_${0..7}.t_order_${0..15}
      databaseStrategy:
        standard:
          shardingColumn: account_id
          shardingAlgorithmName: db_mod_8
      tableStrategy:
        standard:
          shardingColumn: account_id
          shardingAlgorithmName: tb_mod_16
      keyGenerateStrategy:
        column: order_id
        keyGeneratorName: order_seq
    t_order_item:
      actualDataNodes: trade_${0..7}.t_order_item_${0..15}
      databaseStrategy: { standard: { shardingColumn: account_id, shardingAlgorithmName: db_mod_8 } }
      tableStrategy:   { standard: { shardingColumn: account_id, shardingAlgorithmName: tb_mod_16 } }
  bindingTables:
    - t_order,t_order_item          # 同键同片,允许 join
  shardingAlgorithms:
    db_mod_8:  { type: MOD, props: { sharding-count: 8 } }
    tb_mod_16: { type: MOD, props: { sharding-count: 16 } }
  keyGenerators:
    order_seq: { type: ORDER_ID_SEGMENT }   # 自研 SPI:Leaf 段式 + 内嵌分片因子,见 6.6

- !READWRITE_SPLITTING
  dataSources:
    trade_read:
      writeDataSourceName: trade_${0..7}_w
      readDataSourceNames: [trade_${0..7}_r0]
      loadBalancerName: weight_lb
  loadBalancers:
    weight_lb: { type: WEIGHT, props: { trade_0_r0: 100 } }
```
...
```
order_id:  yyyyMMddHH + 机器/分片位(2) + 段内序列(8) + account_id%128 映射位(2)  → 22 位
...
- 备选:ShardingSphere 内置 SNOWFLAKE keygen(内部纯技术 ID 场景可直接用)。
...
| 必须强一致(罕见,如余额与流水) | 约束在同一库同片内设计,靠分片键保证;Seata AT 作为兜底能力预留,默认不开启 |

Outbox relay 为独立轻量服务(Go/Kratos),扫描各分片 outbox 表写 Kafka,保证"DB 写入与事件发布"原子性。

### 6.8 扩容策略

分片容量规划:**8 库 × 16 表 = 128 分片,单分片容量红线 500 万行/50GB**。以 trade_db 估算,支撑百万级付费用户 3~5 年无需扩容。

扩容路径(倍增法,避免重新哈希):

```mermaid
flowchart LR
    A[8库128片] -->|双写开关打开| B[新增8库<br/>目标16库]
    B --> C[历史数据迁移<br/>按 account_id%16 重映射]
    C --> D[全量+增量比对<br/>抽样100%流水核对]
    D --> E[读流量切新库<br/>灰度1%/10%/100%]
    E --> F[写切换停双写<br/>旧库只读保留30天]
```

- 表结构层面 `MOD 16` 不变,库路由从 `%8` 改 `%16`,一半分片原地不动,迁移量减半;
- 迁移与比对工具自研(基于 ShardingSphere 数据迁移能力或 DataX,辅助工具标注:可选);
- 触发条件:任一逻辑库磁盘 > 65% 或单分片行数 > 400 万,启动扩容评审(周期约 4~6 周,纳入《09-roadmap.md》季度规划)。

### 6.9 MySQL 高可用与备份

- **决策:每个逻辑库 1 主 2 从(MGR 单主模式)**,跨 AZ 部署(P2 起);ShardingSphere 读写分离。备选:传统半同步主从 + Orchestrator 自动切主(辅助工具标注:可选)。改选条件:**AZ 间 RTT<2ms 时用 MGR,否则降级半同步**;或团队无 MGR 运维经验且节点预算紧张时,降级为半同步主从(MGR 单主为默认,半同步仅为降级备选,非默认方案)。
...
| 连接池 | HikariCP:`maximumPoolSize ≤ 32`(实例数 × 池大小 ≤ 该库最大连接数的 60%)、`connectionTimeout=3s`、`maxLifetime=25min`、`keepaliveTime=5min` |
...
全局索引表方案(供 OpenAPI 按 resource_id 查询兜底):`resource_db` 中维护 `idx_resource_uid(resource_id, account_id)`,按 resource_id 哈希分 128 片,与主档双写(本地事务,同库);读多写少,容量极小。
...
| 分布式锁 | `lock:billing:deduct:{account_id}` | STRING NX PX + Lua 释放 | 自研封装(Redisson 于 Java 域,可选库),锁必带 TTL 与 owner |
...
| 客户端 | Java:Redisson/Lettuce Cluster 模式;Go:go-redis v9;统一封装:超时 300ms、重试 1 次、熔断降级(缓存不可用时**直通 DB 并限流**) |
...
| MySQL | StatefulSet(主从各一组)或 Operator(辅助工具标注:可选) | 每逻辑库 1 主 2 从 | PVC 500GB 起(云盘/高性能盘) | MGR 单主(跨 AZ,AZ 间 RTT<2ms;否则降级半同步);failover 脚本 + 演练 |
...
| MySQL | 8 库×16 表,每库 500GB | 触发倍增扩容评审 | 16 库;冷热分离入 CK |
...
| Go 域需直连分片 MySQL | 引入 ShardingSphere-Proxy |
| Kafka 磁盘水位 60% / 分区 > 4000 | 扩 broker 或按域拆集群 |
| 单分片行数 > 400 万 | 启动倍增扩容(6.8) |
...
> 4. ShardingSphere-JDBC 分库分表,四库统一 account_id 分片(6.1、6.4)
> 5. 跨库一致性一律"本地事务 + Outbox 事件",Seata 仅预留(6.7)
