...
|---|---|---|---|---|
| 计算 | ECS/ACK/函数计算 | **裁剪**:K8s 为底座,VM 形态(SCECS)一期以 KubeVirt 交付;全品类 Serverless 不做 | 虚机全栈自研成本不可承受,K8s 同时是底座与产品 | 《01》§2.1、《06》、《09》决策 D-03 |
...
| **7** | **计算形态以 K8s 为底座与产品,VM 形态(SCECS)一期以 KubeVirt 交付;资源模型必须现在预留虚机形态** | 全栈虚拟化自研成本不可承受;K8s 同时是底座与产品,KubeVirt 将 VM 抽象为 K8s CRD,一期承接虚机(《09》决策 D-03) | 《06》资源抽象驱动层、《09》一期 KubeVirt |
...
| 13 | 对象存储 | **MinIO**(自建场景;内部使用 + 对外售卖双角色) | S3 兼容、K8s Operator 成熟、自建运维轻 | Ceph / 公有云 OSS | 需块+文件+对象统一存储且规模很大选 Ceph;整体托管上公有云则直接买 OSS(接口 S3 化保证可切换);AGPLv3 许可证风险升级(条款变更/权利方函件,见《09-roadmap.md》风险 R11)时评估 Ceph 或自研对象存储面 | 《04》§9、《09》Build-vs-Buy |
...
>
> **变更登记(2026-08):二期选型实装完成。** 本文件 §4.2 选型表中所列组件(K8s/KubeVirt/APISIX/Vitess/MySQL MGR/Redis Cluster/Kafka KRaft/ClickHouse/Vector/VictoriaMetrics)在二期均已按 source-only 口径落地为代码/manifest,实装状态见《09-roadmap.md》§4.0。选型结论不变,本登记仅补充"已实装"追溯,不改变 §5 变更流程(后续选型变更仍须经架构评审委员会裁决)。
