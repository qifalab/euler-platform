...
|---|---|---|---|---|
| 计算 | ECS/ACK/函数计算 | **裁剪**:K8s 为底座,VM 形态(EUECS)一期以 KubeVirt 交付;全品类 Serverless 不做 | 虚机全栈自研成本不可承受,K8s 同时是底座与产品 | 《01》§2.1、《06》、《09》决策 D-03 |
...
| **7** | **计算形态以 K8s 为底座与产品,VM 形态(EUECS)一期以 KubeVirt 交付;资源模型必须现在预留虚机形态** | 全栈虚拟化自研成本不可承受;K8s 同时是底座与产品,KubeVirt 将 VM 抽象为 K8s CRD,一期承接虚机(《09》决策 D-03) | 《06》资源抽象驱动层、《09》一期 KubeVirt |
