# starcloud Terraform Provider (三期 M-10.2, 验收 C3)

`terraform-provider-starcloud` — the IaC consumer surface for the platform
...
- **One signing implementation.** Every resource CRUD goes through
  `pkg-go/scsdk` → `pkg-go/cps1` — the same signer the gateway verifier, Go
...
- **Secrets stay out of state.** `SC_ACCESS_KEY` / `SC_SECRET_KEY` are read
  from the environment; the provider config surface is endpoint + region only.
- **Resources are thin.** `SCECS/SCOSS/SCVPC/SCRDS` are covered as skeletons
...
|---|---|---|
| SCECS | `starcloud_scecs_instance` | RunInstances / DescribeInstances / TerminateInstances |
| SCOSS | `starcloud_scoss_bucket` | CreateBucket / (DescribeBucket) / DeleteBucket |
| SCVPC | `starcloud_scvpc_vpc` | CreateVpc / — / DeleteVpc |
| SCRDS | `starcloud_scrds_instance` | CreateInstance / — / DeleteInstance |
