...
def sign(
    ...
    hdrs.setdefault(HEADER_DATE, date_value)
...
class Config:
    ...
    security_token: str = ""
    endpoint: str = ""  # overrides the routing subdomain
    timeout: float = 30.0
...
class Client:
    ...
    def call(self, req: ApiRequest) -> ApiResponse:
        ...
        host = self._host_for(req.product_code)
        ...
        body = req.body or b""
        # Build the header set cps1 signs. nonce per call (anti-replay, 07§4.2).
        ...
        if normalized_query:
            url += "?" + urllib.parse.urlencode(normalized_query, doseq=True)
        ...
        try:
            with urllib.request.urlopen(request, timeout=self.config.timeout) as resp:  # noqa: S310
                raw = resp.read()
                return ApiResponse(
            ...
            raw = e.read()
            raise _parse_error(raw, e.code) from None
        except urllib.error.URLError as e:
            raise ApiError("Common.InternalError", 0, str(e)) from None


def _parse_error(body: bytes, status: int) -> ApiError:
    ...
    if code:
        return ApiError(code, status, message or "")
    return ApiError(
