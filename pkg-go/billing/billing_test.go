...
func usage(qty string, covered int) metering.HourlyUsage {
	return metering.HourlyUsage{
		AggID:           metering.AggID("euecs-cn-north-1-01-a1b2c3d4", "cpu_core_hour", billHour),
		AccountID:       100123,
		Region:          "cn-north-1",
		ResourceType:    "ecs",
...
func TestSettleComputesPretax(t *testing.T) {
	...
	s, err := e.Settle(usage("2", 100), amt("0.25"), "euecs", "snap-1", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFractionalQuantityCharging(t *testing.T) {
	...
	s, err := e.Settle(usage("1.99998", 100), amt("0.25"), "euecs", "snap-1", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestChargeIDDerivesFromAggID(t *testing.T) {
	e := newEngine()
	u := usage("2", 100)
	a, _ := e.Settle(u, amt("0.25"), "euecs", "snap-1", nil)
	b, _ := e.Settle(u, amt("0.25"), "euecs", "snap-1", nil)
...
func TestWaterfallOrder(t *testing.T) {
	...
	s, err := e.Settle(usage("2", 100), amt("0.25"), "euecs", "snap-1", pools)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCouponsSpentExpiryAscending(t *testing.T) {
	...
	s, _ := e.Settle(usage("10", 100), amt("0.25"), "euecs", "snap-1", pools)
...
func TestCouponScopeRestriction(t *testing.T) {
	e := newEngine()
	pools := []Pool{
		{Source: SourceCoupon, Ref: "storage-only", Available: amt("10"), ProductCodes: []string{"euoss"}},
		{Source: SourceBalance, Available: amt("100")},
	}
	s, _ := e.Settle(usage("2", 100), amt("0.25"), "euecs", "snap-1", pools)
...
func TestNoPoolsMeansFullShortfall(t *testing.T) {
	e := newEngine()
	s, _ := e.Settle(usage("2", 100), amt("0.25"), "euecs", "snap-1", nil)
...
func TestZeroChargeNeedsNoDeduction(t *testing.T) {
	// Free tiers (EUVPC, EUMON basic) settle to nothing without touching any
	...
	s, err := e.Settle(usage("5", 100), amt("0"), "euvpc", "snap-1", pools)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIncompleteHourIsVisibleOnTheCharge(t *testing.T) {
	e := newEngine()
	s, err := e.Settle(usage("1.5", 75), amt("0.25"), "euecs", "snap-1",
...
func TestFrozenPeriodRejectsSettlement(t *testing.T) {
	...
	_, err := late.Settle(usage("2", 100), amt("0.25"), "euecs", "snap-1", nil)
...
func TestOpenPeriodAllowsSettlement(t *testing.T) {
	...
	if _, err := open.Settle(usage("2", 100), amt("0.25"), "euecs", "snap-1", nil); err != nil {
...
func TestNegativeUnitPriceRejected(t *testing.T) {
	e := newEngine()
	if _, err := e.Settle(usage("2", 100), amt("-0.25"), "euecs", "snap-1", nil); !errors.Is(err, ErrNoUnitPrice) {
...
func TestCostAnalysisBreaksDownByProduct(t *testing.T) {
	charges := []Charge{
		{AccountID: 100123, BillPeriod: "2026-08", ProductCode: "euecs", ResourceID: "r1", PretaxAmount: amt("700"), CoveredRatio: 100},
		{AccountID: 100123, BillPeriod: "2026-08", ProductCode: "euecs", ResourceID: "r2", PretaxAmount: amt("300"), CoveredRatio: 100},
		{AccountID: 100123, BillPeriod: "2026-08", ProductCode: "euoss", ResourceID: "r3", PretaxAmount: amt("50"), CoveredRatio: 100},
		// Other account/period must not leak in.
		{AccountID: 999, BillPeriod: "2026-08", ProductCode: "euecs", ResourceID: "x", PretaxAmount: amt("999"), CoveredRatio: 100},
		{AccountID: 100123, BillPeriod: "2026-07", ProductCode: "euecs", ResourceID: "r1", PretaxAmount: amt("100"), CoveredRatio: 100},
	}
	r := CostAnalysis(100123, "2026-08", charges)
	if r.ByProduct["euecs"] != amt("1000") {
		t.Fatalf("euecs = %s, want 1000", r.ByProduct["euecs"])
	}
	if r.ByProduct["euoss"] != amt("50") {
		t.Fatalf("euoss = %s, want 50", r.ByProduct["euoss"])
	}
...
func TestCostAnalysisAgreesWithSummarize(t *testing.T) {
	...
	charges := []Charge{
		{AccountID: 100123, BillPeriod: "2026-08", ProductCode: "euecs", ResourceID: "r1",
			PretaxAmount: amt("700"), PayAmount: amt("700"), CoveredRatio: 100,
			Deductions: []Deduction{{Source: SourceBalance, Amount: amt("700")}}},
		{AccountID: 100123, BillPeriod: "2026-08", ProductCode: "euoss", ResourceID: "r2",
