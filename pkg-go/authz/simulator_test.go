...
func TestSimulateNoPoliciesIsDefaultDeny(t *testing.T) {
	...
	v := Simulate(nil, "alice", "scecs:StartInstance", "sc:ecs:cn-north-1:100:instance/i-1")
	if v.Allowed {
...
func TestSimulateMatchingAllow(t *testing.T) {
	allow := simPolicy(t, "ScEcsFullAccess", `{
      "Version":"1",
      "Statement":[{"Effect":"Allow","Action":"scecs:*","Resource":"*"}]
    }`)
	v := Simulate([]NamedPolicy{allow}, "bob", "scecs:StartInstance", "sc:ecs:cn-north-1:100:instance/i-1")
	if !v.Allowed {
	...
	if v.DecidingPolicy != "ScEcsFullAccess" {
		t.Fatalf("DecidingPolicy = %q, want ScEcsFullAccess", v.DecidingPolicy)
	}
}
...
func TestSimulateExplicitDenyWinsOverAllow(t *testing.T) {
	...
	allow := simPolicy(t, "ScEcsFullAccess", `{
      "Version":"1","Statement":[{"Effect":"Allow","Action":"scecs:*","Resource":"*"}]
    }`)
	deny := simPolicy(t, "DenyDelete", `{
      "Version":"1","Statement":[{"Effect":"Deny","Action":"scecs:DeleteInstance","Resource":"*"}]
    }`)
	v := Simulate([]NamedPolicy{allow, deny}, "carol", "scecs:DeleteInstance", "sc:ecs:cn-north-1:100:instance/i-1")
	if v.Allowed {
	...
	v2 := Simulate([]NamedPolicy{deny, allow}, "carol", "scecs:DeleteInstance", "sc:ecs:cn-north-1:100:instance/i-1")
	if v2.Allowed || v2.Reason != "explicit_deny" {
		t.Fatalf("deny must win regardless of order: %+v", v2)
	}
	// A non-denied action still allows.
	v3 := Simulate([]NamedPolicy{allow, deny}, "carol", "scecs:StartInstance", "sc:ecs:cn-north-1:100:instance/i-1")
...
func TestSimulateNonMatchingStatementIsDefaultDeny(t *testing.T) {
	...
	p := simPolicy(t, "ReadOnly", `{
      "Version":"1","Statement":[{"Effect":"Allow","Action":"scecs:Describe*","Resource":"*"}]
    }`)
	v := Simulate([]NamedPolicy{p}, "dan", "scecs:DeleteInstance", "sc:ecs:cn-north-1:100:instance/i-1")
	if v.Allowed {
...
func TestSimulateWildcardActionMatchesAny(t *testing.T) {
	...
	for _, act := range []string{"scecs:StartInstance", "scoss:PutObject", "anything:Whatever"} {
		v := Simulate([]NamedPolicy{p}, "eve", act, "sc:ecs:cn-north-1:100:x")
		if !v.Allowed {
...
func TestSimulateDecidingStatementIndexCorrect(t *testing.T) {
	...
	p := simPolicy(t, "Multi", `{
      "Version":"1","Statement":[
        {"Effect":"Allow","Action":"scecs:Describe*","Resource":"*"},
        {"Effect":"Allow","Action":"scecs:StartInstance","Resource":"*"},
        {"Effect":"Deny","Action":"scecs:DeleteInstance","Resource":"*"}
      ]
    }`)
	// StartInstance matches statement 1 (index 1).
	v := Simulate([]NamedPolicy{p}, "gina", "scecs:StartInstance", "sc:ecs:cn-north-1:100:i/1")
	...
	v2 := Simulate([]NamedPolicy{p}, "gina", "scecs:DeleteInstance", "sc:ecs:cn-north-1:100:i/1")
