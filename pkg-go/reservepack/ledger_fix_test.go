...
func TestPurchaseRejectsExistingPackID(t *testing.T) {
	...
	if _, _, err := l.Purchase("pk-dup", 100123, "scecs", "scecs.pack", yuan("1000"), expire, "ord-1"); err != nil {
		t.Fatal(err)
	}
	// Draw the pack down so an overwrite would be visible.
	if _, _, _, err := l.Consume("pk-dup", "scecs", yuan("400"), "bill-1", "c-1"); err != nil {
		t.Fatal(err)
	}

	// A different order key against the same pack id must be rejected.
	if _, _, err := l.Purchase("pk-dup", 100123, "scecs", "scecs.pack", yuan("1000"), expire, "ord-2"); !errors.Is(err, ErrPackExists) {
	...
	pk, entry, err := l.Purchase("pk-dup", 100123, "scecs", "scecs.pack", yuan("1000"), expire, "ord-1")
	if err != nil {
