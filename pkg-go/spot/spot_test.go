...
func TestPublishOpensRecordAndPriceAtReturnsIt(t *testing.T) {
	c := t0
	eng := newEngine(&c)
	p, _ := eng.Publish("euecs", "cn-north-1", yuan("0.50"))
	...
	got, err := eng.PriceAt("euecs", "cn-north-1", t0)
	if err != nil {
...
func TestPublishClosesPreviousRecord(t *testing.T) {
	c := t0
	eng := newEngine(&c)
	_, _ = eng.Publish("euecs", "cn-north-1", yuan("0.50"))
	c = t0.Add(10 * time.Minute)
	_, _ = eng.Publish("euecs", "cn-north-1", yuan("0.40"))

	hist, err := eng.History("euecs", "cn-north-1")
	if err != nil {
...
func TestAppendOnlyDoesNotRewriteHistory(t *testing.T) {
	...
	_, _ = eng.Publish("euecs", "cn-north-1", yuan("0.30"))
	...
	p, err := eng.PriceAt("euecs", "cn-north-1", t0.Add(5*time.Minute))
	if err != nil {
		t.Fatalf("priceAt: %v", err)
	}
	if p.PricePerHour != yuan("0.50") {
	...
	// And t0+15min → 0.40.
	p, err = eng.PriceAt("euecs", "cn-north-1", t0.Add(15*time.Minute))
	if err != nil {
		t.Fatalf("priceAt: %v", err)
	}
...
func TestPriceAtRejectsFutureCycle(t *testing.T) {
	...
	// not been set yet.
	if _, err := eng.PriceAt("euecs", "cn-north-1", t0.Add(time.Hour)); err == nil {
...
func TestPriceAtRejectsUnknownProduct(t *testing.T) {
	c := t0
	eng := newEngine(&c)
	if _, err := eng.PriceAt("euecs", "cn-north-1", t0); err == nil {
...
func TestCurrentReturnsLatestPrice(t *testing.T) {
	...
	c = t0.Add(10 * time.Minute)
	_, _ = eng.Publish("euecs", "cn-north-1", yuan("0.40"))
	p, err := eng.Current("euecs", "cn-north-1")
	if err != nil {
