--
-- sc-audit: audit side-stream plugin (07-security.md §6.1/§6.2, 03§4.4.3).
--
...
--      than being skipped when sc-authorize rejects.
--
...
local _M = {
    version  = 0.1,
    priority = 2300,   -- after sc-authorize (2400)
    name     = plugin_name,
    schema   = schema,
}
...
-- ak_id form in the audit event schema (07§6.1: "SC****3F").
local function mask_ak(ak)
    if not ak or #ak < 4 then
        return nil
    end
    return "SC****" .. string.sub(ak, -2)
end
...
function _M.log(conf, ctx)
    ...
    -- Unauthenticated requests that never reached sc-auth carry no identity;
    ...
    local event = {
        ...
        event_name   = ctx.sc_action or ngx.req.get_uri_args()["Action"],
        source_ip    = ctx.var.remote_addr,
        user_agent   = core.request.header(ctx, "User-Agent"),
        identity = {
            account_id  = account_id,
            principal   = ctx.sc_identity,
            ak_id       = mask_ak(ctx.sc_ak_id),
            -- MFA presence is carried on the identity when the console path
            -- established it; absent for signature auth.
            mfa_present = ctx.sc_mfa_present or false,
        },
        resource        = ctx.sc_resource_arn and { ctx.sc_resource_arn } or {},
        decision        = decision,
        decision_number = ctx.sc_decision_number,
        ...
        -- described in 07§6.2.
        trace_id = core.request.header(ctx, "X-Sc-TraceId") or ngx.var.request_id,
    }
    ...
    if not err and payload then
        ...
        if not ok then
            core.log.error("sc-audit: kafka emit failed: ", kafka_err)
        end
    else
        core.log.error("sc-audit: encode failed: ", err)
    end
end
