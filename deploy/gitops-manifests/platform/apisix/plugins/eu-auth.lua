--
-- eu-auth: OpenAPI authentication plugin (07-security.md §5, 04§3.4).
--
...
--   * OpenAPI zone ({productCode}.api.euler.emoera.com): CPS1 signature
--   * Console zone (console.euler.emoera.com): JWT issued by svc-iam
--
-- On success the plugin injects the identity headers upstream services trust
-- (07§3.3): X-Euler-Account-Id, X-Euler-Identity, X-Euler-TraceId, plus X-Euler-Ak-Id and
-- X-Euler-Quota-Qps for the per-AK rate limiter that runs after this plugin.
--
...
-- therefore strips any client-supplied X-Euler-* header before injecting its own.
--
...
local schema = {
    ...
    properties = {
        ...
        -- file path to the mounted eu-jwt-public-key Secret
...
local _M = {
    version  = 0.1,
    priority = 2500,   -- after limit-req (2600), before eu-authorize (2400)
    name     = plugin_name,
    schema   = schema,
}
...
local function get_jwt_public_key(conf)
    ...
    if not f then
        core.log.error("eu-auth: cannot read jwt_public_key_file ", path, ": ", err)
...
-- strip_client_identity_headers removes any X-Euler-* header the client sent.
-- Without this, a caller could present X-Euler-Account-Id and impersonate another
-- tenant on any upstream that trusts the header (07§4.3 内部防伪).
local function strip_client_identity_headers(ctx)
    local headers = ngx.req.get_headers()
    for name, _ in pairs(headers) do
        if string.sub(string.lower(name), 1, 5) == "x-eu-" then
...
local function verify_signature(conf, ctx)
    ...
    if not httpc then
        core.log.error("eu-auth: failed to create http client: ", err)
    ...
    if not body then
        core.log.error("eu-auth: cannot read request body for signature verification")
    ...
    if not res then
        ...
        core.log.error("eu-auth: verify endpoint unreachable: ", req_err)
        return deny(ctx, 503, "Common.ServiceUnavailable", "auth backend unavailable")
    end
    ...
    -- eu-authorize plugin that runs next.
    local account_id = res.headers["X-Euler-Account-Id"]
    local ak_id      = res.headers["X-Euler-Ak-Id"]
    local identity   = res.headers["X-Euler-Identity"]

    if not account_id then
        core.log.error("eu-auth: verify returned 200 without X-Euler-Account-Id")
        return deny(ctx, 503, "Common.InternalError", "auth backend contract violation")
    end

    core.request.set_header(ctx, "X-Euler-Account-Id", account_id)
    core.request.set_header(ctx, "X-Euler-Ak-Id", ak_id)
    if identity then
        core.request.set_header(ctx, "X-Euler-Identity", identity)
    end
    core.request.set_header(ctx, "X-Euler-TraceId", ngx.var.request_id)

    -- Stash on ctx so eu-authorize and the audit side-stream can read them
    -- without re-parsing headers.
    ctx.eu_account_id = account_id
    ctx.eu_ak_id      = ak_id
    ctx.eu_identity   = identity

    -- Per-AK quota for the limit-count plugin keyed on X-Euler-Ak-Id (04§3.5).
    local quota = res.headers["X-Euler-Quota-Qps"]
    if quota then
        core.request.set_header(ctx, "X-Euler-Quota-Qps", quota)
    end
    return nil
end
...
local function verify_jwt(conf, ctx)
    ...
    if payload.principal then
        core.request.set_header(ctx, "X-Euler-Identity", payload.principal)
    end
    core.request.set_header(ctx, "X-Euler-TraceId", ngx.var.request_id)

    ctx.eu_account_id = account_id
    ctx.eu_identity   = payload.principal
    return nil
end
