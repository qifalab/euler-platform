--
-- sc-authorize: coarse-grained authorization plugin (07-security.md §3.4/§5).
--
...
--
--   Gateway (here):  authentication (done by sc-auth) + interface-level
...
--
-- High-sensitivity interfaces (SCECS delete, SCOSS download, RAM changes) are
...
local plugin_name = "sc-authorize"
...
local schema = {
    ...
    properties = {
        ...
        -- placeholders, e.g.
        --   sc:ecs:$region:$account_id:instance/$arg_InstanceId
...
local _M = {
    version  = 0.1,
    priority = 2400,   -- immediately after sc-auth (2500)
    name     = plugin_name,
    schema   = schema,
}
...
local function call_check_access(conf, ctx, account_id, identity, action, resource_arn)
    ...
    if not httpc then
        core.log.error("sc-authorize: http client: ", err)
        return nil, "backend"
    end
    ...
    local payload = {
        ...
        context    = {
            ["sc:SourceIp"]    = ctx.var.remote_addr,
            ["sc:CurrentTime"] = os.date("!%Y-%m-%dT%H:%M:%SZ"),
        },
    }
    ...
    if not res then
        core.log.error("sc-authorize: check endpoint unreachable: ", req_err)
        return nil, "backend"
    end
...
function _M.access(conf, ctx)
    ...
    -- sc-auth must have run and established the identity. If it did not, the
    -- route is misconfigured; fail closed rather than treating the request as
    -- anonymous.
    local account_id = ctx.sc_account_id
                       or core.request.header(ctx, "X-Sc-Account-Id")
    if not account_id then
        core.log.error("sc-authorize: no identity on ctx; is sc-auth configured on this route?")
    ...
    local identity = ctx.sc_identity or core.request.header(ctx, "X-Sc-Identity") or ""
    ...
    ctx.sc_action          = action
    ctx.sc_resource_arn    = resource_arn
    ctx.sc_decision_number = decision.decision_number
    return nil
end
