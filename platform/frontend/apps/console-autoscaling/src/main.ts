/**
 * console-autoscaling — Wujie sub-app entry (02§3.4).
...
export async function mount(el: HTMLElement | string) {
  ...
  if (!root) throw new Error("[console-autoscaling] mount target not found");
...
export async function beforeLoad() {
  console.debug("[console-autoscaling] beforeLoad");
}
