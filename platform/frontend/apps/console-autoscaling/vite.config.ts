    ...
    proxy: {
      ...
      "/console": {
        ...
        configure: (p) => p.on("proxyReq", (r) => {
          ...
          r.setHeader("X-Euler-TraceId", `scaling-${Date.now().toString(36)}`);
        }),
      },
      // Real provisioning → svc-orchestrator (03§4.3, the 开通 step). EUAS flows
      // through the same saga as every product — productCode=euas routes it to
      ...
      // Product catalogue + 询价 → svc-catalog (01§7, the quote link). EUAS's
      // per-hour management-fee pricing rule lives here (M-7.3).
      "/api/v1/catalog": {
