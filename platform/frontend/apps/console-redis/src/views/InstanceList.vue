...
/**
 * Instance list (02§7.2). The managed-Redis sub-app's main view.
 * ResourceTable + StatusBadge + polling on transitional states. Filtered to
 * euredis — the same /console/resources endpoint the BFF serves for every
...
const { rows, loading, columns, page, pageSize, total, setPage } = useResourceTable<Row>({
  api: async () => {
    ...
    const items = (res.data ?? [])
      .filter((r) => r.ProductCode === "euredis")
      .map((r) => ({
...
<template>
  <section class="redis-app">
    <PageHeader title="云数据库 Redis 版">
      <template #actions>
    ...
    <EmptyGuide v-if="!loading && rows.length === 0" title="暂无 Redis 实例" description="创建您的第一个托管 Redis,主备跨可用区,平台运维经验产品化。" action-label="创建实例" action-href="#/buy" />
  </section>
</template>

<style scoped>
.redis-app { padding: 16px 24px; }
</style>
