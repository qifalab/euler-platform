 ...
 * is the authoritative source of state, spec, billing start, etc. SCKAFKA
 * carries chargeType PREPAID or POSTPAID (managed services offer both).
 */
  ...
  <section class="detail">
    ...
    <ElTabs v-else v-model="activeTab" class="detail-tabs">
      <ElTabPane label="监控" name="monitor">
        <ElEmpty description="监控指标(Broker 吞吐/分区延迟/消息堆积)由云监控(svc-monitor)提供,该端点接入后在此展示" />
      </ElTabPane>
