...
/**
 * Instance list (02§7.2). The managed-Kafka sub-app's main view.
 * ResourceTable + StatusBadge + polling on transitional states. Filtered to
 * sckafka — the same /console/resources endpoint the BFF serves for every
...
const { rows, loading, columns, page, pageSize, total, setPage } = useResourceTable<Row>({
  api: async () => {
    ...
    const items = (res.data ?? [])
      .filter((r) => r.ProductCode === "sckafka")
      .map((r) => ({
        ...
        spec: r.SpecCode,
        zoneId: r.Region,
        chargeType: r.ChargeType === "PREPAID" ? "包年包月" : "按量",
      }));
...
<template>
  <section class="kafka-app">
    <PageHeader title="托管 Kafka">
      <template #actions>
    ...
    <EmptyGuide v-if="!loading && rows.length === 0" title="暂无 Kafka 实例" description="创建您的第一个托管 Kafka,broker 跨可用区,平台运维经验产品化。" action-label="创建实例" action-href="#/buy" />
  </section>
</template>

<style scoped>
.kafka-app { padding: 16px 24px; }
</style>
