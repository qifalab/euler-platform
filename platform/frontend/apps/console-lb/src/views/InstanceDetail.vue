 ...
 * is the authoritative source of state, spec, billing start, etc. SCLB carries
 * chargeType=POSTPAID (按量) — the only form a load balancer is sold in.
 */
...
interface ResourceDetail {
  ...
  productCode: string;
  region: string;
  chargeType: string; // POSTPAID (usage-billed)
...
const chargeLabel = computed(() => inst.value?.chargeType === "POSTPAID" ? "按量(LCU+流量)" : "—");
</script>
  ...
  <section class="detail">
    ...
    <ElTabs v-else v-model="activeTab" class="detail-tabs">
      <ElTabPane label="监控" name="monitor">
        <ElEmpty description="监控指标(LCU 使用率/流量/健康检查)由云监控(svc-monitor)提供,该端点接入后在此展示" />
      </ElTabPane>
      ...
      <ElTabPane label="网络" name="network">
        <ElEmpty description="网络配置(VPC/监听器/后端服务器组)由 svc-vpc 与 rc-lb 提供,该端点接入后在此展示" />
      </ElTabPane>
