    ...
    proxy: {
      ...
      "/console": {
        ...
        configure: (p) => p.on("proxyReq", (r) => {
          ...
          r.setHeader("X-Euler-TraceId", `lb-${Date.now().toString(36)}`);
        }),
      },
      // Real provisioning → svc-orchestrator (03§4.3, the 开通 step). EULB flows
      // through the same saga as every product — productCode=eulb routes it to
      ...
      // Product catalogue + 询价 → svc-catalog (01§7, the quote link). EULB's
      // usage-based hourly pricing rules live here (M-7.2).
      "/api/v1/catalog": {
