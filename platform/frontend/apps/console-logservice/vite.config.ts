    ...
    proxy: {
      ...
      "/console": {
        ...
        configure: (p) => p.on("proxyReq", (r) => {
          ...
          r.setHeader("X-Sc-TraceId", `sclog-${Date.now().toString(36)}`);
        }),
      },
      // Real provisioning → svc-orchestrator (03§4.3, the 开通 step). SCLOG
      // flows through the same saga as every product — productCode=sclog
      ...
      // Product catalogue + 询价 → svc-catalog (01§7, the quote link). SCLOG's
