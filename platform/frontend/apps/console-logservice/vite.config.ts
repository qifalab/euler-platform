    ...
    proxy: {
      ...
      "/console": {
        ...
        configure: (p) => p.on("proxyReq", (r) => {
          ...
          r.setHeader("X-Euler-TraceId", `eulog-${Date.now().toString(36)}`);
        }),
      },
      // Real provisioning → svc-orchestrator (03§4.3, the 开通 step). EULOG
      // flows through the same saga as every product — productCode=eulog
      ...
      // Product catalogue + 询价 → svc-catalog (01§7, the quote link). EULOG's
      // per-HOUR + USAGE pricing rules live here (M-7.5).
      "/api/v1/catalog": {
