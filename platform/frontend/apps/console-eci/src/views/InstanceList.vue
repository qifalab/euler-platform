...
/**
 * Instance list (02§7.2). The ECI sub-app's main view.
 * ResourceTable + StatusBadge + polling on transitional states. Filtered to
 * sceci — the same /console/resources endpoint the BFF serves for every
...
<template>
  <section class="eci-app">
    <PageHeader title="弹性容器实例 ECI">
      <template #actions>
    ...
    <EmptyGuide v-if="!loading && rows.length === 0" title="暂无容器实例" description="创建您的第一个弹性容器实例,按秒计费,秒级拉起。" action-label="创建实例" action-href="#/buy" />
  </section>
</template>

<style scoped>
.eci-app { padding: 16px 24px; }
</style>
