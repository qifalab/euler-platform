    ...
    proxy: {
      ...
      "/console": {
        ...
        configure: (p) => p.on("proxyReq", (r) => {
          ...
          r.setHeader("X-Euler-TraceId", `eci-${Date.now().toString(36)}`);
        }),
      },
      // Real provisioning → svc-orchestrator (03§4.3, the 开通 step). ECI flows
      // through the same saga as every product — productCode=eueci routes it to
      ...
      // Product catalogue + 询价 → svc-catalog (01§7, the quote link). EUECI's
