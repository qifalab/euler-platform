    ...
    proxy: {
      ...
      "/console": {
        ...
        configure: (p) => p.on("proxyReq", (r) => {
          ...
          r.setHeader("X-Euler-TraceId", `backup-${Date.now().toString(36)}`);
        }),
      },
      // Real provisioning → svc-orchestrator (03§4.3, the 开通 step). EUBACKUP
      // flows through the same saga as every product — productCode=eubackup
      ...
      // Product catalogue + 询价 → svc-catalog (01§7, the quote link). EUBACKUP's
      // per-HOUR pricing rules live here (M-7.4).
      "/api/v1/catalog": {
