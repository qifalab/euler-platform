 ...
 *
 * Wired to the real backend chain (same shape as console-eci, productCode=eubackup):
 *  - GET  /api/v1/catalog/skus?productCode=eubackup — spec catalogue
 ...
 *
 * EUBACKUP is POSTPAID ONLY and REGIONAL (a backup policy is regional; cross-AZ
 * copy is a policy flag, not a placement constraint), so there is NO zone
 * picker. The catalogue's per-HOUR pricing rule returns the hourly payable as
 * payableAmount; the summary shows 预估每小时. Price is server-trial-computed;
 * the client never invents a unit price. The order id is issued by svc-order.
 */
...
// --- region metadata (catalogue-driven, no hardcoded lists). EUBACKUP is
// REGIONAL: no zone picker at all, so only the region list is needed. ---
const { regions, load } = useCatalogMeta("eubackup");
...
interface Sku {
  skuCode: string;
  productCode: string;
  chargeType: string; // POSTPAID only for EUBACKUP
  specJson: string;
  status: string;
}
...
const specs = computed(() => {
  // EUBACKUP has only postpaid SKUs. The spec list is the SKU list, labelled
  // by tier (standard / crossaz).
  return skus.value.map((s) => {
    const spec = parseSpec(s.specJson);
    const label = spec.tier === "crossaz" ? "跨可用区备份" : "标准备份";
    return { code: s.skuCode, label, tier: spec.tier, crossAz: !!spec.cross_az };
  });
});
...
function parseSpec(json: string): SkuSpec {
  try {
    ...
    return { tier: p.tier ?? "standard", cross_az: p.cross_az };
  } catch {
...
onMounted(async () => {
  try {
    const res = await sdk.get<Sku[]>("/api/v1/catalog/skus?productCode=eubackup");
    skus.value = res.data ?? [];
    if (specs.value.length && !form.value.spec) {
      form.value.spec = specs.value[0].code;
      form.value.tier = specs.value[0].tier;
...
// When the spec changes, sync the tier + crossAz flag (the crossaz edition
// turns the cross-AZ-copy intent on).
watch(() => form.value.spec, (code) => {
  const entry = specs.value.find((s) => s.code === code);
  if (entry) {
    form.value.tier = entry.tier;
...
async function refreshQuote() {
  ...
  try {
    const res = await sdk.post<QuoteResult>("/api/v1/catalog/quote", {
      ...
      // REGIONAL product: NO zoneId (a backup policy spreads; zone is not its concern).
    });
...
async function submit() {
  const specCode = form.value.spec;
  if (!specCode) { ElMessage.error("请选择备份规格"); active.value = 0; return; }
  if (form.value.scheduleHours <= 0) { ElMessage.warning("备份间隔必须大于 0"); active.value = 1; return; }
  if (form.value.retentionDays < 0) { ElMessage.warning("保留期不能为负"); active.value = 1; return; }
  ...
  try {
    ...
    const created = await sdk.post<{ orderId: number; orderNo: string; state: string }>(
      "/api/v1/orders",
      {
        type: "NEW", productCode: "eubackup", skuCode: specCode,
    ...
    // The productCode=eubackup routes it to the DriverK8s binding. The policy
    // params (schedule/retention/crossAz) flow through to the reconcile loop,
    // which reads them via pkg-go/backup to decide when to back up + what to expire.
    const res = await sdk.post<{ resourceId: string; state: string; orderState: string }>(
      "/api/v1/orchestrator/fulfill",
      {
        ...
        productCode: "eubackup", region: form.value.region,
        specCode, chargeType: "POSTPAID",
        targetResourceType: form.value.targetResourceType,
        scheduleHours: form.value.scheduleHours,
        retentionDays: form.value.retentionDays,
        crossAz: form.value.crossAz,
      },
    );
    ElMessage.success(`备份策略已创建:${res.data.resourceId.slice(0, 24)}…(状态 ${res.data.state})`);
    ...
    <div class="buy-layout">
      <div class="buy-main">
        <ElSteps :active="active" align-center class="buy-steps">
          <ElStep title="基础配置" /><ElStep title="备份策略" /><ElStep title="确认下单" />
        </ElSteps>
        ...
        <ElForm v-show="active === 0" label-position="top" class="buy-form">
          ...
          <p class="form-hint">备份策略为 REGIONAL 产品:策略是地域级的,跨 AZ 副本是策略标志(非放置约束)。</p>
        </ElForm>

        <ElForm v-show="active === 1" label-position="top" class="buy-form">
          <ElFormItem label="备份目标资源类型">
            <ElSelect v-model="form.targetResourceType">
              <ElOption value="instance" label="云服务器 ECS" />
              <ElOption value="dbinstance" label="云数据库 RDS" />
              <ElOption value="disk" label="云盘" />
            </ElSelect>
          </ElFormItem>
          <ElFormItem label="备份间隔 (小时)"><ElInputNumber v-model="form.scheduleHours" :min="1" :max="720" /></ElFormItem>
          <ElFormItem label="保留期 (天)"><ElInputNumber v-model="form.retentionDays" :min="0" :max="3650" /></ElFormItem>
          <ElFormItem label="跨可用区备份">
            <ElSwitch v-model="form.crossAz" :disabled="form.tier !== 'crossaz'" />
            <span class="form-hint-inline">{{ form.crossAz ? "开启:快照将复制到第二可用区" : "关闭:仅单可用区快照" }}</span>
          </ElFormItem>
        </ElForm>
        ...
        <div v-show="active === 2" class="buy-confirm">
          ...
          <ul class="confirm-list">
            ...
            <li>目标资源类型:{{ form.targetResourceType }}</li>
            <li>备份间隔:每 {{ form.scheduleHours }} 小时</li>
            <li>保留期:{{ form.retentionDays }} 天{{ form.retentionDays === 0 ? "(永久)" : "" }}</li>
            <li>跨可用区备份:{{ form.crossAz ? "是" : "否" }}</li>
            <li>计费方式:按量付费</li>
          </ul>
        </div>
      ...
      <aside class="buy-summary">
        ...
        <dl class="sum-list">
          ...
          <div><dt>间隔</dt><dd>每 {{ form.scheduleHours }} 小时</dd></div>
        ...
        <p class="sum-hint">按存储容量按量计费;价格由服务端试算</p>
      </aside>
    </div>
  </section>
</template>
