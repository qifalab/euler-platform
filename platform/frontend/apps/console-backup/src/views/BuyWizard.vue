 ...
 *
 * Wired to the real backend chain (same shape as console-eci, productCode=scbackup):
 *  - GET  /api/v1/catalog/skus?productCode=scbackup — spec catalogue
 ...
 *
 * SCBACKUP is POSTPAID ONLY and REGIONAL (a backup policy is regional; cross-AZ
...
interface Sku {
  skuCode: string;
  productCode: string;
  chargeType: string; // POSTPAID only for SCBACKUP
  specJson: string;
  status: string;
}
...
const specs = computed(() => {
  // SCBACKUP has only postpaid SKUs. The spec list is the SKU list, labelled
...
onMounted(async () => {
  try {
    const res = await sdk.get<Sku[]>("/api/v1/catalog/skus?productCode=scbackup");
    skus.value = res.data ?? [];
...
async function submit() {
  ...
  try {
    ...
    const created = await sdk.post<{ orderId: number; orderNo: string; state: string }>(
      "/api/v1/orders",
      {
        type: "NEW", productCode: "scbackup", skuCode: specCode,
    ...
    // The productCode=scbackup routes it to the DriverK8s binding. The policy
    ...
    const res = await sdk.post<{ resourceId: string; state: string; orderState: string }>(
      "/api/v1/orchestrator/fulfill",
      {
        ...
        productCode: "scbackup", region: form.value.region,
