...
| Site | Domain | Role |
|---|---|---|
| Marketing portal | `www.starcloud.cn` | Nuxt3 SSR (SEO) |
| Docs site | `docs.starcloud.cn` | VitePress SSG |
| Console shell | `console.starcloud.cn` | SPA (Vue3 + Vite), Wujie base |
| Account center | `account.starcloud.cn` | SSO-only auth domain |
| Billing center | `billing.starcloud.cn` | Billing / orders |
| Ticket support | `ticket.starcloud.cn` | Work orders / support |
...
- Route: first path segment = productCode (e.g. `/scecs/instances`); sub-app owns the rest.

## Auth (SSO)

- Dual token: short-lived `access_token` (JWT, 15 min, memory-only) + long-lived
  `refresh_token` (opaque, 7 days, HttpOnly cookie, `Domain=.starcloud.cn`).
...
## Shared packages (`@sc/*`, private registry)

`@sc/ui`, `@sc/tokens`, `@sc/sdk`, `@sc/console-kit`, `@sc/wujie-bridge` —
consumed by version, built via externals sharing (no runtime Module Federation
for phase 1).

## Design tokens (`@sc/tokens`)

CSS variables `--sc-*`: color/brand, bg, text, font-size, spacing, radius/shadow,
layout (`--sc-topbar-height: 56px`, `--sc-sider-width: 208px`). Dark mode via
`[data-theme="dark"]`. Base component library: Element Plus, thin-wrapped by `@sc/ui`.
...
- TypeScript strict, pnpm workspaces, Node LTS, Conventional Commits,
  Pinia (single instance), requests only via `@sc/sdk`, i18n zh-CN first.
