...
--    scecs/scrds/scredis/sckafka = ZONAL(创建时锁定一个 AZ,M-6 的 placement 门
...
ALTER TABLE `t_product`
  ...
  ADD COLUMN `cross_az` TINYINT NOT NULL DEFAULT 0
    COMMENT 'M-6: ZONAL 产品的跨可用区副本(HA)标记;REGIONAL 下仅 sclb(跨AZ流量入口)为 1'
    AFTER `region_scope`;

UPDATE `t_product` SET `region_scope` = 'REGIONAL', `cross_az` = 0
  WHERE `product_code` IN ('scvpc','scoss','scmon','scas','scbackup','sclog');

UPDATE `t_product` SET `region_scope` = 'REGIONAL', `cross_az` = 1
  WHERE `product_code` = 'sclb';

UPDATE `t_product` SET `region_scope` = 'ZONAL', `cross_az` = 1
  WHERE `product_code` IN ('scecs','scrds','scredis','sckafka');

UPDATE `t_product` SET `region_scope` = 'ZONAL', `cross_az` = 0
  WHERE `product_code` IN ('scbs','sceip','sceci');
