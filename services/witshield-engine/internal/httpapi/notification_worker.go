...
func (s *Server) deliverNotification(workerCtx context.Context, delivery store.NotificationDelivery) {
	operationCtx, stopOperation := s.operationContext(workerCtx)
	defer stopOperation()
	attemptCtx, cancel := context.WithTimeout(operationCtx, notificationAttemptTimeout)
	...
	defer recordCancel()
	if workerCtx.Err() != nil || !s.operationEnabled(workerCtx) {
