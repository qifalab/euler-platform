...
import (
	...
	"github.com/qifalab/euler-platform/services/witshield-engine/internal/ids"
	"github.com/qifalab/euler-platform/services/witshield-engine/internal/notification"
...
func (s *Server) putNotificationSettings(w http.ResponseWriter, r *http.Request) {
	...
	// Euler keeps stored credentials bound to the destination the administrator
	// actually configured. Editing a destination must never silently forward an
	// existing secret to a new host, SMTP account, or webhook path.
	if old.EncryptedWebhookSecret != "" && strings.TrimSpace(in.WebhookURL) != old.Settings.WebhookURL && in.WebhookSecret == nil && !in.ClearWebhookSecret {
		writeError(w, 400, "endpoint_credentials_required", "changing the webhook endpoint requires re-entering or explicitly clearing its signing secret")
		return
	}
	smtpDestinationChanged := !strings.EqualFold(strings.TrimSpace(in.SMTPHost), old.Settings.SMTPHost) || in.SMTPPort != old.Settings.SMTPPort || in.SMTPUsername != old.Settings.SMTPUsername
	if old.EncryptedSMTPPassword != "" && smtpDestinationChanged && in.SMTPPassword == nil && !in.ClearSMTPPassword {
		writeError(w, 400, "endpoint_credentials_required", "changing SMTP host, port or username requires re-entering or explicitly clearing its password")
		return
	}
