...
func TestCancelledInstallationClosesBlockedSMTPSession(t *testing.T) {
	listener, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		t.Fatal(err)
	}
	...
	sender, err := New(Config{Settings: domain.NotificationSettings{SMTPEnabled: true, SMTPHost: "127.0.0.1", SMTPPort: listener.Addr().(*net.TCPAddr).Port, SMTPFrom: "sender@example.test", SMTPTo: []string{"recipient@example.test"}}})
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		finished <- sender.SendChannel(ctx, domain.NotificationSMTP, domain.NotificationEvent{ID: "event-cancelled", Title: "test"})
	}()
	select {
	case conn := <-accepted:
		defer conn.Close()
	case <-time.After(2 * time.Second):
		t.Fatal("SMTP connection was not made")
	}
	// Server deliberately never sends its SMTP greeting. Context cancellation
	// must interrupt net/smtp's blocking ReadResponse without waiting 20 seconds.
	cancel()
	select {
	case err := <-finished:
		if err == nil {
			t.Fatal("cancelled delivery returned success")
		}
	case <-time.After(2 * time.Second):
		t.Fatal("cancelled SMTP session remained connected")
	}
}
