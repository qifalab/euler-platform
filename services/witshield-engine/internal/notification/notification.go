...
func (s *Sender) email(ctx context.Context, event domain.NotificationEvent) error {
	...
	defer conn.Close()
	// net/smtp does not propagate contexts after DialContext returns. Closing the
	// connection on cancellation prevents a disabled Euler installation from
	// continuing a blocked SMTP session until the ordinary deadline expires.
