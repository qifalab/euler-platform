...
func TestDisabledInstallationCannotQueueScheduledCommands(t *testing.T) {
	now := time.Now().UTC()
	f := &fakeStore{due: []domain.Schedule{{ID: "s", DeviceID: "d", Kind: domain.ScheduleScan, Interval: time.Hour, NextRunAt: now.Add(-time.Hour)}}}
	s := New(f, nil)
	enabled := false
	s.SetAllowed(func(context.Context) bool { return enabled })
	if err := s.Tick(context.Background()); err != nil {
		t.Fatal(err)
	}
	if len(f.commands) != 0 {
		t.Fatal("disabled installation dispatched scan")
	}
	enabled = true
	if err := s.Tick(context.Background()); err != nil {
		t.Fatal(err)
	...
	if len(f.commands) != 1 {
		t.Fatal("reenabled installation failed to resume scheduler")
	}
}
